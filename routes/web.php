...
Route::post('/login', function () {
    ...
    $A=DB::table('mhsrtb')->where($u)->first();
    if(empty($A->Umdp)){
...
Route::post('/regis', function () {
    ...
    $A=DB::table('mhsrtb')->where(['Name' => $_POST['Nama'],'Kelas' => $_POST['Kelas']])->get();
    if(count($A)>0){
...
Route::get('/coreuv', function () {
    $d=DB::table('mhsrtb')->where('Kelas', session()->get('Kelas'))->get();
    $data = [];
